...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x, n_seq, n_event=None):
        """
        x : (n_batch x n_seq x n_dim)
        pos_enc : (1 x n_seq x n_dim) -> (1 x n_seq x n_dim )
        """
...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_time, src_mask, n_seq=None, return_attns=False):
        '''
        src_seq : (n_batch x seq_len x n_event)
        src_time : (n_batch x seq_len x d_clock)
        '''
    
        enc_slf_attn_list = []
        
        # NOTE: this is where pooling happen (n_event -> d_word_vec)
        # item_emb : (n_batch x seq_len x d_word_vec)
        ...
        if self.use_pos_enc:
            enc_output = self.position_enc(enc_output, n_seq=n_seq)
...
class Transformer(nn.Module):
    ...
    def forward(self, src_seq, trg_seq=None, seq_time_idx=None,
        ...
        # n_seq x n_seq
        subseq_mask = get_subsequent_mask(n_seq, device=src_seq.device)

        # (n_batch x [n_seq*n_events] x d_emb)
        zeros_inp = torch.zeros(n_b, n_seq, self.d_word_vec).to(src_seq.device)
        inp_time_enc = self.position_enc(zeros_inp, n_seq=n_seq)
        ...
        enc_output = self.encoder(src_seq, inp_time_enc, subseq_mask, n_seq=n_seq)

        """
        output whole time-steps (n_batch x n_seq x n_events)
        """

        seq_logit = self.predictor(enc_output, subseq_mask, trg_time_enc, None, len_seq)
...
if __name__ == '__main__':
    ...
    # model param
    padding_idx = 0
    tf_d_word = tf_d_model = tf_d_k = tf_d_inner = 6
    num_layers = 4
    num_heads = 2
    dropout = 0.2
    use_pos_enc = False    
