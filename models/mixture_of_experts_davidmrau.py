...
# FROM https://github.com/davidmrau/mixture-of-experts/blob/master/moe.py
...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, gates):
        ...
        # NOTE: sorted_experts = location of nonzero elements (3d)
...
class SeqMoE(nn.Module):
    ...
    def __init__(
        ...
        noisy_gating=True, k=4, dropout=0.2
    ):
        ...
        self.experts = nn.ModuleList(
            [GRUPredictor(embed_size, self.hidden_size, self.output_size, dropout)
                for i in range(self.num_experts)])
        self.w_gate = nn.Parameter(torch.zeros(
            embed_size, num_experts), requires_grad=True)
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        top_logits, top_indices = logits.topk(
            min(self.k + 1, self.num_experts), dim=2)
