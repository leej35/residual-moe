    ...
    def __init__(self, in_features, out_features, filter_square_matrix, bias=True):
        ...
        if use_gpu:
            self.filter_square_matrix = Variable(
                filter_square_matrix.cuda(), requires_grad=False)
        else:
            self.filter_square_matrix = Variable(
...
class GRUD(nn.Module):
    def __init__(self, input_size, cell_size, hidden_size, X_mean, output_last=False):
        ...
        if use_gpu:
            ...
            self.X_mean = Variable(torch.Tensor(X_mean).cuda()) if (X_mean is not None) else None
        else:
            self.identity = torch.eye(input_size)
            self.zeros = Variable(torch.zeros(input_size))
            self.X_mean = Variable(torch.Tensor(X_mean)) if (X_mean is not None) else None
    ...
    def forward(self, input):
        ...
        for i in range(step_size):
            Hidden_State = self.step(torch.squeeze(X[:, i:i+1, :]), torch.squeeze(X_last_obsv[:, i:i+1, :]), torch.squeeze(
                self.X_mean[:, i:i+1, :]), Hidden_State, torch.squeeze(Mask[:, i:i+1, :]), torch.squeeze(Delta[:, i:i+1, :]))
            if outputs is None:
    ...
    def register_xmean(self, x_mean):
        use_gpu = torch.cuda.is_available()
        if use_gpu:
            self.X_mean = Variable(torch.Tensor(x_mean).cuda()) if (x_mean is not None) else None
        else:
